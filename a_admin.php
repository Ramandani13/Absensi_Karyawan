...
?>
<h1><b><center>DATA KEHADIRAN GRUP A</center></b></h1>
...
<div><button style="position:relative;margin-left:20px;"><a href="b_admin.php"><h style="color:red;">pindah group b</h></a></button></div>
<div class="scroll2">
...
<form id="in-bar" method="get" action="" class="fill-barcode">
			<p style="margin-left: 2%;">
				...
				<!-- <center><input style="position:relative;background-color:#44566C;border-radius:5px;color:white;" type="text" name="dani1" placeholder="Search NPK" ><center>
			...
			<td><input type="submit"  value="OK" hidden=""></td>
                                    ...
                                    <?php
                                        ...
                                        if(isset($_GET['dani'])){
                                            $dtgl = $_GET['dani'];
                                            // $vu = $_GET['dani1'];
                                        $ambildata = mysqli_query($koneksi, "SELECT * FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$dtgl' and hadir = 'O'  ");
                                        // $ambildata = mysqli_query($koneksi, "SELECT * FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and vu = '$vu' and hadir = 'O'  ");
                                        }else{
                                            $ambildata = mysqli_query($koneksi, "SELECT * FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and hadir = 'O'  ");
                                        }
