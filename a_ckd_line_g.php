...
?>  
    ...
    <h1 class="card-title mb-0" style="color:red;font-size:30px;text-align:center;"><b>ABSENSI CKD LINE G GROUP A</b></h1><br>
...
$set_jam = '60'; 
    ...
    // TOTAL HADIR                                        
     ...
     $yanghadir = mysqli_query($koneksi, "SELECT  COUNT(hadir) AS yghadir FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and hadir = 'O' and lokasi = 'CKD_LINE_G' GROUP BY hadir ");
    while($yanghadirabsen = mysqli_fetch_array($yanghadir)) {  
    $totalhadirabsen = $yanghadirabsen['yghadir'];
    }

    // SAKIT
    $yangsakit = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygsakit FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and catatan = 'SAKIT' and lokasi = 'CKD_LINE_G' GROUP BY catatan ");
    while($yangsakita = mysqli_fetch_array($yangsakit)) {  
    $totalsakit = $yangsakita['ygsakit'];
    }

    // IZIN
    $yangizin = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygizin FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and catatan = 'IZIN' and lokasi = 'CKD_LINE_G' GROUP BY catatan ");
    while($yangizina = mysqli_fetch_array($yangizin)) {  
    $totalizin = $yangizina['ygizin'];
    }

    // CUTI
    $yangcuti = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygcuti FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and catatan = 'CUTI' and lokasi = 'CKD_LINE_G' GROUP BY catatan ");
    while($yangcutia = mysqli_fetch_array($yangcuti)) {  
    $totalcuti = $yangcutia['ygcuti'];
    }

    // ALFA
    $totallnya = mysqli_query($koneksi, "SELECT  COUNT(vu) AS totall FROM tb_absen where lokasi = 'CKD_LINE_G' ");
...
<!-- <a style="color:red; font-weight: bold;position:relative;margin-left:1000px;" href="delete_all.php" >DELETE ALL</a> -->
                    ...
                    <?php
                      ...
                      $no=1;
                        $sql1 = mysqli_query($koneksi,"SELECT * FROM tb_absen where lokasi = 'CKD_LINE_G' ");
...
{   
        ...
        if($cek > 0){
            ...
            echo '<META HTTP-EQUIV="Refresh" Content="0; URL=a_ckd_line_g.php">';
