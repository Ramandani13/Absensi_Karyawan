...
?>	
    ...
    <h1 class="card-title mb-0" style="color:red;font-size:30px;text-align:center;"><b>ABSENSI GROUP A</b></h1><br>
...
<div id="wrapper">  
   ...
   <button style="position:relative;margin-left:20px;"><a href="a_admin.php"><h style="color:red;">ADMIN</h></a></button>
   <style type="text/css">
#gradient3 {
...
$set_jam = '60'; 
											...
											$tgl2=date("Y-m-d");
    // TOTAL HADIR                                        
    // $yanghadir = mysqli_query($koneksi, "SELECT  COUNT(hadir) AS yghadir FROM tb_absen_record WHERE tgl_key = '$tanggal_ori' GROUP BY hadir ");
    // while($yanghadirabsen = mysqli_fetch_array($yanghadir)) {  
    // $totalhadirabsen = $yanghadirabsen['yghadir'];
    // }

     $yanghadir = mysqli_query($koneksi, "SELECT  COUNT(hadir) AS yghadir FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and hadir = 'O' GROUP BY hadir ");
    while($yanghadirabsen = mysqli_fetch_array($yanghadir)) {  
    $totalhadirabsen = $yanghadirabsen['yghadir'];
    }

    // SAKIT
    $yangsakit = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygsakit FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and catatan = 'SAKIT' GROUP BY catatan ");
    while($yangsakita = mysqli_fetch_array($yangsakit)) {  
    $totalsakit = $yangsakita['ygsakit'];
    }

    // IZIN
    $yangizin = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygizin FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and catatan = 'IZIN' GROUP BY catatan ");
    while($yangizina = mysqli_fetch_array($yangizin)) {  
    $totalizin = $yangizina['ygizin'];
    }
    // CUTI
    $yangcuti = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygcuti FROM tb_absen_record INNER JOIN tb_absen ON tb_absen_record.vu = tb_absen.vu and tgl_key = '$tanggal_ori' and catatan = 'CUTI' GROUP BY catatan ");
...
<!-- <a style="color:red; font-weight: bold;position:relative;margin-left:1000px;" href="delete_all.php" >DELETE ALL</a> -->
                    ...
                    <?php
                      ...
                      if(isset($_GET['carinama'])){
                        ...
                        $sql1 = mysqli_query($koneksi, "SELECT * FROM tb_absen WHERE vu = '$carinama'  ");
                        }else{
                        $sql1 = mysqli_query($koneksi,"SELECT * FROM tb_absen");
                        }
