...
?>	
    ...
    <h1 class="card-title mb-0" style="color:blue;font-size:30px;text-align:center;"><b>ABSENSI GROUP B</b></h1><br>
...
<div id="wrapper">  
    <div id="blue">
     <div id="tanggal"></div>		
     <div style="position:relative;left:1600px;top:-40px;"><a href="../index.php"><img style="border-radius:20px;" src="image/orb.gif" width="50px" height="50px;"></a>	</div>		 
   </div>
   
   <button style="position:relative;margin-left:20px;"><a href="data_zona_b.php"><h style="color:red;">Data Zona</h></a></button>
   <button style="position:relative;margin-left:20px;"><a href="b_admin.php"><h style="color:red;">ADMIN</h></a></button>
   <style type="text/css">
#gradient3 {
...
$set_jam = '60'; 
											...
											$tgl2=date("Y-m-d");
    // TOTAL HADIR                                        
    // $yanghadir = mysqli_query($koneksi, "SELECT  COUNT(hadir) AS yghadir FROM tb_absen_b_record WHERE tgl_key = '$tanggal_ori' GROUP BY hadir ");
    // while($yanghadirabsen = mysqli_fetch_array($yanghadir)) {  
    // $totalhadirabsen = $yanghadirabsen['yghadir'];
    // }

    $yanghadir = mysqli_query($koneksi, "SELECT  COUNT(hadir) AS yghadir FROM tb_absen_b_record INNER JOIN tb_absen_b ON tb_absen_b_record.vu = tb_absen_b.vu and tgl_key = '$tanggal_ori' and hadir = 'O' GROUP BY hadir ");
    while($yanghadirabsen = mysqli_fetch_array($yanghadir)) {  
    $totalhadirabsen = $yanghadirabsen['yghadir'];
    }

    // SAKIT
    $yangsakit = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygsakit FROM tb_absen_b_record WHERE tgl_key = '$tanggal_ori' and catatan = 'SAKIT' GROUP BY catatan ");
    while($yangsakita = mysqli_fetch_array($yangsakit)) {  
    $totalsakit = $yangsakita['ygsakit'];
    }
    // IZIN
    $yangizin = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygizin FROM tb_absen_b_record WHERE tgl_key = '$tanggal_ori' and catatan = 'IZIN' GROUP BY catatan ");
    while($yangizina = mysqli_fetch_array($yangizin)) {  
    $totalizin = $yangizina['ygizin'];
    }
    // CUTI
    $yangcuti = mysqli_query($koneksi, "SELECT  COUNT(catatan) AS ygcuti FROM tb_absen_b_record WHERE tgl_key = '$tanggal_ori' and catatan = 'CUTI' GROUP BY catatan ");
    while($yangcutia = mysqli_fetch_array($yangcuti)) {  
    $totalcuti = $yangcutia['ygcuti'];
    }
    // ALFA
    $totallnya = mysqli_query($koneksi, "SELECT  COUNT(vu) AS totall FROM tb_absen_b ");
...
<form id="in-bar" method="get" action="" class="fill-barcode">
			<p style="margin-left: 2%;">
				<center><input style="position:absolute;background-color:white;border-radius:5px;color:#0000CD;" type="text" name="carinama" placeholder="Cari NPK"><center>
				<div id="toggle" onclick="showHide();"></div>
			</p>
			<td><input type="submit"  value="OK" hidden=""></td>
</form>
<a style="font-weight: bold;position:relative;color:black;" href="importan_b/form.php">IMPORT DATA </a>
<a style="font-weight: bold;position:relative;margin-left:1200px;" href="index.php"><span style="color:red;">Untuk pindah group ,klik ...</span>GROUP A</a>
...
<!-- <a style="color:red; font-weight: bold;position:relative;margin-left:1000px;" href="delete_all.php" >DELETE ALL</a> -->
                    ...
                    <?php
                      ...
                      if(isset($_GET['carinama'])){
                        ...
                        $sql1 = mysqli_query($koneksi, "SELECT * FROM tb_absen_b WHERE vu = '$carinama'  ");
                        }else{
                        $sql1 = mysqli_query($koneksi,"SELECT * FROM tb_absen_b ");
                        }
...
while ($data = mysqli_fetch_array($sql1)) {
		...
		$sql2[$u] = mysqli_query($koneksi,"SELECT * FROM tb_absen_b_record WHERE vu='$vu[$u]' AND tgl_key='$tgl2' ");
		...
		$catatan[$u] = $data1['catatan'];
			...
			for ($i=0 ; $i < $jumlah_baris ; $i++){
				...
				<td><?= $catatan[$i] ?></td>
                <td><a href='edit_absen_b.php?id=<?= $id[$i] ?>'>Edit</a></td></tr>
				</tr>
			<?php } ?>	
			...
			<p style="margin-left: 2%;">
				<input style="position:absolute;background-color:#F8F8FF;border-radius:5px;" type="text" name="dani" placeholder="Tapping..." autofocus>
				<div id="toggle" onclick="showHide();"></div>
			</p>
...
{   
		...
		$nama = $_POST['dani'];
		$panggildb= mysqli_query($koneksi, "SELECT * FROM tb_absen_b WHERE vu = '$nama'");
        ...
        $cek = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM tb_absen_b_record WHERE cekdatasama='$cekdatasama' "));

        if($cek > 0){
            echo '<script>alert("GAGAL ...!")</script>';
        }else{
            $sql="INSERT INTO tb_absen_b_record (id,hadir,ot,op_pengganti,pembimbing,pengecek,catatan,vu,tgl_key,cekdatasama) VALUES ('$id2','O','$ot','$op_pengganti','$pembimbing','$pengecek','$catatan','$vu','$tgl2','$cekdatasama')";  
            ...
            echo '<META HTTP-EQUIV="Refresh" Content="0; URL=index_b.php">';
